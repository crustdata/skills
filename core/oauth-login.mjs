 ...
 * Shared core: `runLogin()` is the login flow; each plugin ships a thin bin/
 * launcher that calls it (Claude: `bin/crustdata-login.mjs`), e.g.
 *
...
// Import-only (shared across plugins): the runnable launcher is bin/crustdata-login.mjs,
// which imports and calls runLogin(). Tests import the exported functions directly.
