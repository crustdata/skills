 ...
 * Fallback: GET <base>/.well-known/oauth-protected-resource (RFC 9728) → take
 * its first authorization server → fetch THAT server's RFC 8414 metadata.
 * Returns the metadata object or null. Never throws.
 */
export async function discover(baseUrl, fetchImpl = globalThis.fetch) {
  const base = String(baseUrl).replace(/\/+$/, "");
  const direct = await fetchJson(fetchImpl, `${base}/.well-known/oauth-authorization-server`);
  if (isUsableMetadata(direct)) return direct;
  ...
  for (const server of servers) {
    ...
    const asBase = server.replace(/\/+$/, "");
    const meta = await fetchJson(fetchImpl, `${asBase}/.well-known/oauth-authorization-server`);
    if (isUsableMetadata(meta)) return meta;
  }
  return null;
}
 ...
 * resolves the `code` (state-checked) or rejects on error/timeout.
 */
function startLoopback(expectedState) {
  return new Promise((resolveStart, rejectStart) => {
    ...
    const server = createServer((req, res) => {
      ...
      if (err !== null || code === null || state !== expectedState) {
        const reason =
          err !== null
