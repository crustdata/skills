#!/usr/bin/env node
/**
 * Crustdata login launcher for Codex — a thin shell over the shared OAuth flow
 * in ../../core/oauth-login.mjs (the same one the Claude plugin ships). Writes
 * the token to the shared `~/.crustdata` store the SessionStart hook reads.
 *
