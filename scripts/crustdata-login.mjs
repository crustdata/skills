#!/usr/bin/env node
/** OAuth 2.1 code + PKCE (S256), public client. Only a 4-char token tail is ever printed. */
...
/** RFC 7636. 32 random bytes give a 43-char verifier, the floor of the spec's 43-128 window. */
...
/** Loopback http is for the e2e harness; a poisoned discovery doc must not get plaintext. */
...
/** RFC 8414 §3.3: the issuer identifies the server, so pin it and every endpoint to the doc's origin. */
...
/** RFC 8414 metadata, falling back to the RFC 9728 protected-resource doc. Never throws. */
...
/** RFC 7591 registration where the server advertises it, else a static `client_id` from metadata. */
...
/** WSL reports platform "linux" while the browser lives on the Windows side. */
...
/** Pure so the platform branches are testable without spawning anything. */
export function browserCommand(url, platform = process.platform, env = process.env) {
  ...
  // WSL usually has no working xdg-open; powershell.exe is always on its PATH, and '' escapes a quote.
...
/** Serves exactly one authorization redirect, on a random loopback port. */
function startLoopback(expectedState, { expectedIssuer, requireIss = false } = {}) {
  return new Promise((resolveStart, rejectStart) => {
    ...
    const server = createServer((req, res) => {
      ...
      // RFC 9207: a mismatched `iss` means a different AS than the one we authorized against.
    ...
    server.listen(0, "127.0.0.1", () => {
      ...
      resolveStart({
        ...
        waitForCode: () =>
          outcome.finally(
            () =>
              // Exiting mid-close trips a libuv assert on Windows (Node 24, src/win/async.c).
              new Promise((done) => {
                clearTimeout(timer);
                server.close(() => done());
...
if (invokedAs !== "" && fileURLToPath(import.meta.url) === invokedAs) {
  // Detached so a Claude Code `/command` can fire login without shell backgrounding.
  ...
  // A hard exit races libuv's teardown of the loopback server on Windows; the timer is a backstop.
